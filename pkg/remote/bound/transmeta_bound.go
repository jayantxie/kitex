...
func (h *svrMetaHandler) OnStreamWrite(ctx context.Context, st streaming.ServerStream, sendMsg remote.Message) (context.Context, error) {
	var err error
...
func (h *svrMetaHandler) OnStreamRead(ctx context.Context, st streaming.ServerStream, msg remote.Message) (nctx context.Context, err error) {
	for _, hdlr := range h.mhs {
...
func (h *cliMetaHandler) OnStreamWrite(ctx context.Context, st streaming.ClientStream, sendMsg remote.Message) (context.Context, error) {
	var err error
	for _, hdlr := range h.mhs {
		ctx, err = hdlr.WriteMeta(ctx, sendMsg)
		if err != nil {
...
func (h *cliMetaHandler) OnStreamRead(ctx context.Context, st streaming.ClientStream, msg remote.Message) (nctx context.Context, err error) {
	for _, hdlr := range h.mhs {
		ctx, err = hdlr.ReadMeta(ctx, msg)
		if err != nil {
			return ctx, err
