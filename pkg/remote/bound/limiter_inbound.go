...
// OnStream implements the remote.ServerPipelineHandler interface.
...
func (l *serverLimiterHandler) OnStreamRead(ctx context.Context, st streaming.ServerStream, args remote.Message) (context.Context, error) {
	if l.qpsLimitPostDecode {
		if l.qpsLimit.Acquire(ctx) {
			return ctx, nil
		}
...
func (l *serverLimiterHandler) OnStreamWrite(ctx context.Context, st streaming.ServerStream, args remote.Message) (context.Context, error) {
	return ctx, nil
}
