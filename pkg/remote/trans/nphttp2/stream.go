...
type serverStreamX struct {
	...
	grpcStreamWithMW *streamWithMiddleware
}

func (s *serverStreamX) GetGRPCStream() streaming.Stream {
	if s.grpcStreamWithMW != nil {
		return s.grpcStreamWithMW
	}
	return s.grpcStream
}

func (s *serverStreamX) InjectGRPCMiddlewares(recvEP endpoint.RecvEndpoint, sendEP endpoint.SendEndpoint) {
	s.grpcStreamWithMW = newStreamWithMiddleware(s.grpcStream, recvEP, sendEP)
}
...
func (s *clientStreamX) InjectGRPCMiddlewares(recvEP endpoint.RecvEndpoint, sendEP endpoint.SendEndpoint) {
	s.grpcStreamWithMW = newStreamWithMiddleware(s.grpcStream, recvEP, sendEP)
}

// newClientStream ...
func newClientStream(ctx context.Context, svcInfo *serviceinfo.ServiceInfo, conn *clientConn, t *cliTransHandler) streaming.ClientStream {
	sx := &clientStreamX{
		ctx:     ctx,
		svcInfo: svcInfo,
		conn:    conn,
