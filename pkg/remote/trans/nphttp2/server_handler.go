...
func (t *svrTransHandler) handleFunc(s *grpcTransport.Stream, svrTrans *SvrTrans, conn net.Conn) {
	...
	// add gRPC recv/send middleware
	streamx.InjectGRPCMiddlewares(t.opt.Streaming.BuildRecvInvokeChain(invokeRecvEndpoint()),
		t.opt.Streaming.BuildSendInvokeChain(invokeSendEndpoint()))
...
func (t *svrTransHandler) OnStream(ctx context.Context, st remote.ServerStream) error {
