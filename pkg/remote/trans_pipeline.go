...
type ServerPipelineHandler interface {
	...
	OnStream(ctx context.Context, st ServerStream) (context.Context, error)
...
func (p *ServerTransPipeline) OnStream(ctx context.Context, st ServerStream) error {
	var err error
	for _, h := range p.svrHdrls {
...
func (p *ClientTransPipeline) OnConnectStream(ctx context.Context) (nctx context.Context, err error) {
	for _, h := range p.cliHdrls {
