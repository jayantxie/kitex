...
type ServerPipelineHandler interface {
	...
	OnStreamRead(ctx context.Context, st streaming.ServerStream, msg Message) (nctx context.Context, err error)
	OnStreamWrite(ctx context.Context, st streaming.ServerStream, send Message) (nctx context.Context, err error)
}

type ClientPipelineHandler interface {
	OnInactive(ctx context.Context, conn net.Conn) context.Context
	OnConnectStream(ctx context.Context) (nctx context.Context, err error)
	OnStreamRead(ctx context.Context, st streaming.ClientStream, msg Message) (nctx context.Context, err error)
	OnStreamWrite(ctx context.Context, st streaming.ClientStream, send Message) (nctx context.Context, err error)
}
...
func (p *ServerTransPipeline) OnStreamWrite(ctx context.Context, st streaming.ServerStream, send Message) (nctx context.Context, err error) {
	for _, h := range p.svrHdrls {
		ctx, err = h.OnStreamWrite(ctx, st, send)
		if err != nil {
			return ctx, err
		}
	}
...
func (p *ServerTransPipeline) OnStreamRead(ctx context.Context, st streaming.ServerStream, msg Message) (nctx context.Context, err error) {
	for _, h := range p.svrHdrls {
		ctx, err = h.OnStreamRead(ctx, st, msg)
		if err != nil {
			return ctx, err
		}
	}
...
func (p *ClientTransPipeline) OnStreamWrite(ctx context.Context, st streaming.ClientStream, send Message) (nctx context.Context, err error) {
	for _, h := range p.cliHdrls {
		ctx, err = h.OnStreamWrite(ctx, st, send)
		if err != nil {
			return ctx, err
...
func (p *ClientTransPipeline) OnStreamRead(ctx context.Context, st streaming.ClientStream, msg Message) (nctx context.Context, err error) {
	for _, h := range p.cliHdrls {
		ctx, err = h.OnStreamRead(ctx, st, msg)
		if err != nil {
			return ctx, err
